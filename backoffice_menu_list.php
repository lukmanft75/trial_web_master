...
?>
	...
	<?=$f->input("add","Add","type='button' onclick=\"window.location='backoffice_menu_add.php';\"");?>
	<?=$paging;?>
	...
	<?php foreach($backoffice_menu as $no => $backoffice_men){ ?>
		<?php
			$actions = /* "<a href=\"backoffice_menu_view.php?id=".$backoffice_men["id"]."\">View</a> |  */
						"<a href=\"backoffice_menu_edit.php?id=".$backoffice_men["id"]."\">Edit</a> |
						<a href='#' onclick=\"if(confirm('Are You sure to delete this data?')){window.location='?deleting=".$backoffice_men["id"]."';}\">Delete</a>
						";
		?>
		<?=$t->row(
					array($no+$start+1,
