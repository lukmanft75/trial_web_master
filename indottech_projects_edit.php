...
<?php
	if(isset($_POST["save"])){
		$db->addtable("indottech_projects");$db->where("id",$_GET["id"]);
	...
	$db->addtable("indottech_projects");$db->where("id",$_GET["id"]);$db->limit(1);$indottech_project = $db->fetch_data();
	$txt_initial = $f->input("initial",$indottech_project["initial"]);
	$txt_name = $f->input("name",$indottech_project["name"]);
	
?>
<?=$f->start("","POST","","enctype='multipart/form-data'");?>
	<?=$t->start("","editor_content");?>
		<?=$t->row(array("Project Initial",$txt_initial));?>
