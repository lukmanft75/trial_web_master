...
<?php $is_amandemen = $db->fetch_single_data("joborder","is_amandemen",array("id"=>$_GET["id"]));?>
<div class="bo_title" id="bo_title"> Edit Job Order <?php if($is_amandemen == 1){ echo "(Amandemen)"; } else { echo "(Extension)";}?> </div>
<?php
	if(isset($_POST["save"])){
		...
		if($updating["affected_rows"] >= 0){
			$db->addtable("joborder_allowances"); $db->where("joborder_id",$_GET["id"]); $db->delete_();
			$db->addtable("joborder_benefits"); $db->where("joborder_id",$_GET["id"]); $db->delete_();
