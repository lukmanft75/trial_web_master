...
<div class="bo_title">Edit Tax Document</div>
<?php
	if(isset($_POST["save"])){
		$db->addtable("tax_documents");			$db->where("id",@$_GET["id"]);
		...
		if($updating["affected_rows"] >= 0){
			$tax_document_id = $_GET["id"];
			if($_FILES["softcopy"]["tmp_name"]){
				$_ext = strtolower(pathinfo($_FILES['softcopy']['name'],PATHINFO_EXTENSION));
				$softcopy_name = $tax_document_id.".".$_ext;
				move_uploaded_file($_FILES['softcopy']['tmp_name'],"files_pajak/".$softcopy_name);
				$db->addtable("tax_documents");	$db->where("id",$tax_document_id);
	...
	$db->addtable("tax_documents");$db->where("id",$_GET["id"]);$db->limit(1);$tax_document = $db->fetch_data();
	
	$name 					= $f->input("name",$tax_document["name"],"size='50'");
	$storage_position 		= $f->input("storage_position",$tax_document["storage_position"]);
	$softcopy 				= $tax_document["softcopy"]."<br>".$f->input("softcopy","","type='file'");
	$received_by_indohr_at 	= $f->input("received_by_indohr_at",$tax_document["received_by_indohr_at"],"type='date'");
?>
