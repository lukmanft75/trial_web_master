...
<div class="bo_title">Add Job Order</div>
<?php
	if(isset($_POST["save"])){
	...
	if(isset($_GET["copyid"])){ $db->addtable("joborder");$db->where("id",$_GET["copyid"]);$db->limit(1);$jo = $db->fetch_data(); }
...
<?php
	if(isset($_GET["copyid"])){ 
		$db->addtable("joborder_allowances");$db->where("joborder_id",@$_GET["copyid"]);
		...
		$db->addtable("joborder_benefits");$db->where("joborder_id",@$_GET["copyid"]);
		foreach($db->fetch_data(true) as $key => $_data){
			?>
				<script> 
