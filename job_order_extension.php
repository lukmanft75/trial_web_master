...
<div class="bo_title" id="bo_title"> Add Job Order <?php if($_GET["is_amandemen"] == 1){ echo "(Amandemen)"; } else { echo "(Extension)";}?> </div>
<?php
	$_GET["id"] = $_GET["joborder_id"];
	if(isset($_POST["save"])){
		$db->addtable("joborder");
		$db->addfield("joborder_id");			$db->addvalue(@$_GET["joborder_id"]);
		if($_GET["is_amandemen"] == 1){
			$db->addfield("is_amandemen");		$db->addvalue("1");
			$db->addfield("pkwt_for");			$db->addvalue("amandemen");
		}else{
			$db->addfield("pkwt_for");			$db->addvalue("extension");
		}
		...
		if($inserting["affected_rows"] >= 0){
			...
			foreach($_POST["allowances"] as $key => $allowance_id){
				if($allowance_id > 0){
					...
					$db->addfield("joborder_id");		$db->addvalue($insert_id);
			...
			javascript("window.location='".str_replace("_extension","_list",$_SERVER["PHP_SELF"])."';");
		} else {
	...
	if($_GET["is_amandemen"] == 1){ $sel_pkwt_for = "Amandemen"; } else { $sel_pkwt_for = "Extension";}
    ...
    $txt_remarks = $f->textarea("remarks",$jo["remarks"]);
